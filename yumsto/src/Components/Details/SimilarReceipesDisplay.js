...
if (similarReceipesApiData){
    if (similarReceipesApiData.length > 0){
        return similarReceipesApiData.map((receipe,idx)=>{
            ...
            return(
                <Link style={{textDecoration:"none"}} to={`/details/${receipe.id}`} key={idx}>
                <div onClickCapture={dummystateupdate} className="similarReceipesCol">
                    
                    <img src={`https://spoonacular.com/recipeImages/${receipe.id}-240x150.jpg`} />
                    <p>{receipe.title}</p>
                </div>
                </Link>
            )
    ...
    return(
        ...
        <div className="similarReceipesRow">
            
            {renderSimilarReceipes()}
